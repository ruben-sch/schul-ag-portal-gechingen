...
def send_single_leader_email(ag):
    """
    Sends the detailed email (participants + waitlist) to the AG leader.
    Returns True if sent successfully, False otherwise.
    """
    ...
    if not participants.exists() and not waitlist.exists():
        return False
